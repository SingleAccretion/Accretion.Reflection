...
- `MethodInfo.DefaultValue` is a very tricky API:
  - It returns whatever is in the metadata if there is metadata (`.param [i]` directive in IL). This means that a parameter of type `Int32` can easily have a default value of `Byte` if some CLI compiler decides to optimize the assembly size this way.
